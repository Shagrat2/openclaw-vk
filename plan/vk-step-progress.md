...
### Ш10 · CHANGELOG + версия ✅ (запись добавлена; версию бампить при релизе)
- `CHANGELOG.md`: секция `## Unreleased`, `### Добавлено` + блоки «Кому важно» и
  «Что проверить после обновления» — стиль как в записи v2026.6.19. Номер версии и
  `package.json version` — на момент релиза (не бампил).
