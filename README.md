...
| `streaming` | Черновик шагов, как `streaming.mode: "progress"` у Telegram: при `mode: "progress"` шаги агента (вызовы инструментов, поиск) показываются в одном сообщении, которое правится на месте и в конце становится ответом. Заголовок черновика задаётся `progress.label`, остальные ключи `progress` — общие для каналов OpenClaw. Задаётся на весь канал; внутри `accounts` этот блок не допускается. По умолчанию выключен. |
