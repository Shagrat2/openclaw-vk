...
- `src/question.ts`: questions from the core (`ask_user`, `AskUserQuestion`) — reading them from a payload, the inline callback keyboard, where each was delivered; text answers are claimed by the core itself
- `src/question-events.ts`: `message_event` (pressed question button) → the core's question resolver, with a snackbar answer
